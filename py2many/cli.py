...
class JavascriptTranspiler(CLikeTranspiler):
    NAME = "JavaScript"
    BASE = DartTranspiler
    cmd = ["dartdevc", "--modules=node", "{filename}", "-o", "{output}"]
    ...
    def visit(self, tree):
        ...
        with open(js_file) as f:
            ...
            # Allow using with dartdevc which doesnt include this line
            if self.cmd[0] == "dart2js":
                assert generated_by.startswith("// Generated by dart2js")
                version_start = generated_by.find(" version")
                assert version_start != -1, generated_by
