...
class CppTranspiler(CLikeTranspiler):
    ...
    def headers(self, meta: InferMeta):
        self._headers.append('#include "py14/runtime/sys.h"  // NOLINT(build/include_order)')
        self._headers.append('#include "py14/runtime/builtins.h"  // NOLINT(build/include_order)')
        if self.use_catch_test_cases:
            self._headers.append('#include "py14/runtime/catch.hpp"  // NOLINT(build/include_order)')
    ...
    def visit_ClassDef(self, node):
        ...
        buf += ["public: // NOLINT(whitespace/indent)"]
        fields = []
        index = 0
    ...
    def _dispatch(self, node, fname: str, vargs: List[str]) -> Optional[str]:
        def visit_range(node, vargs: List[str]) -> str:
            self._headers.append('#include "py14/runtime/range.hpp"  // NOLINT(build/include_order)')
            args = ", ".join(vargs)
