...
from py2many.inference import get_element_types, get_inferred_type
...
class RustTranspiler(CLikeTranspiler):
    ...
    def visit_Assign(self, node):
        ...
        if isinstance(target, ast.Name) and defined_before(definition, node):
            ...
            key_types = get_element_types(node.value.keys, node.scopes, self._type_map)
            if len(set(key_types)) == 1 and key_types[0] is not None:
                key_typename = key_types[0]
            else:
                key_typename = "_"
            value_types = get_element_types(
                node.value.values, node.scopes, self._type_map
            )
